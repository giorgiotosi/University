...
public class Insurance {
	...
	public Insurance(LocalDate start, LocalDate end) {
		...
		dates = new LocalDate[end.getDayOfYear() - start.getDayOfYear()+1];
		int i = 0 ;
	...
	private int costFor(LocalDate when) {
		if (Season.of(when) == Season.WINTER) {
			return 100;
		}
		else if (Season.of(when) == Season.SUMMER) {
			return 20;
		}
		else
			return 50;
	...
	public long cost() {
		...
		for(LocalDate date : dates) {
			sum += costFor(date);
			System.out.println(Season.of(date));
		}
		return sum;
	}
