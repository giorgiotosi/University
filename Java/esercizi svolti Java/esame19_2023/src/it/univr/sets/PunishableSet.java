...
public class PunishableSet<E> implements Iterable<E> {
	...
	public boolean add(E element) {
		...
		if(container.containsKey(element)) {
			return false; // element è già nell'insieme
		}
		if(init.applyAsInt(element)<= 0) {
			return false; //il valore iniziale dato dalla funzione init non e' positivo
		}
	...
	public void punish(E element, int points) {
		// COMPLETARE
		if(/*!container.containsKey(element) || */points < 0) {
			throw new IllegalArgumentException();
		}
		if(!container.containsKey(element)) { return;}
		container.put(element, container.get(element)-points);
	 ...
	 */
	@Override
	public String toString() { // FATTO, NON MODIFICARE
		String result = "";
