...
public class QuarterDate extends Date {
	...
	public QuarterDate(int day, Quarter quarter, int year) {
		super(daysAfterStartOf1900(day, quarter, year));
	}

	public QuarterDate(int i) {
		super(i);
	}
	...
	private static int daysAfterStartOf1900(int day, Quarter quarter, int year) {
	 // TODO. A cosa potrebbe servirvi questo metodo privato?
		int days = 0;
		days += day-1;
		for(int i = 0; i< quarter.ordinal(); i++) {
			days += daysInQuarter(Quarter.values()[i], year);
		}
	...
	public String toString() {
		...
		result = String.format("%02d%s %d", getDay(), getQuarter(), getYear());
	...
	public QuarterDate after(int days) {
		return new QuarterDate(days+ daysAfterStartOf1900(getDay(),getQuarter(),getYear())); 		
	}
}