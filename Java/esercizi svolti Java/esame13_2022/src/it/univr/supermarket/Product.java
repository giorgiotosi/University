...
public abstract class Product implements Comparable<Product> {
	...
	public final boolean equals(Object other) {
		return other instanceof Product && name.equals(((Product) other).name) 
				&& price == ((Product) other).price && compareExpiration((Product) other)== 0;
	}
	...
	public final boolean expiresInSubsequent24Hours(long whenChecked) {
		if(!hasExpired(whenChecked) && hasExpired(whenChecked + _24_HOURS)) {
			return true;
		}
		else
			return false;
